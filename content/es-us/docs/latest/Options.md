...
which by default is stored in the [?user]({{<relref path="Aegisub_path_specifiers" lang="en">}}) directory.
...
: The duration added to the start of a line by the "add lead-in" and "add
  lead-out" functions. Also used in (and set by) the [timing postprocessor]({{<relref path="Timing_Post-Processor" lang="en">}}).
...
: Decides where Aegisub should save screenshots. The default is `?video`, which
  ...
  path you like. [Aegisub_path_specifiers]({{<relref path="Aegisub_path_specifiers" lang="en">}}) are supported; another option
...
click the New button. Enter the [command name]({{<relref path="Commands" lang="en">}}), then set the hotkey
...
: Decides what backend Aegisub uses to render subtitles on the video.
  ...
  *CSRI/vsfilter_textsub* (Windows only)
  ...
  : Use [libass](http://code.google.com/p/libass/) to render subtitles.
    ...
    that will be [softsubbed]({{<relref path="Attaching_subtitles_to_video#softsubbing" lang="en">}}) it
